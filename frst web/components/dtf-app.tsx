'use client';
/* oxlint-disable next/no-img-element -- DTF previews use local object URLs or authenticated Netlify Blob URLs. */
...
import { designFileUrl } from '@/lib/library-types';
...
const nav: { label: View; icon: typeof Home }[] = [
  ...
  { label: 'Biblioteca de diseños', icon: FolderOpen },
...
const garmentSizes = 'XS,S,M,L,XL,2XL,3XL,4XL,5XL';
const productTemplates = [
  ...
  { name: 'Musculosa', sku: 'MUSC', category: 'Prendas', sizes: garmentSizes },
  {
    name: 'Remera regular',
    sku: 'REM-REG',
    category: 'Prendas',
  ...
  { name: 'Gorra', sku: 'GORRA', category: 'Accesorios', sizes: 'Único' },
  { name: 'Mate', sku: 'MATE', category: 'Objetos', sizes: 'Único' },
  { name: 'Taza', sku: 'TAZA', category: 'Objetos', sizes: 'Único' },
];
...
export default function DtfApp({ userName }: { userName: string }) {
  ...
  const [builderSeed, setBuilderSeed] = useState<LibraryDesign | null>(null);
  const [globalQuery, setGlobalQuery] = useState('');

  const globalResults = useMemo(() => {
    const term = globalQuery.trim().toLocaleLowerCase();
    if (term.length < 2) return [];
    return [
      ...data.orders
        .filter((order) =>
          [order.number, order.customerName, order.items]
            .join(' ')
            .toLocaleLowerCase()
            .includes(term),
        )
      ...
      ...data.customers
        ...
        .map((customer) => ({
          ...
          view: 'Clientes' as View,
          title: customer.name,
          meta: customer.phone || 'Cliente',
        })),
      ...data.products
        .filter((product) =>
        ...
        .map((product) => ({
          ...
          view: 'Productos y stock' as View,
          title: product.name,
          meta: product.sku,
        })),
      ...data.designs
        .filter((design) =>
          [design.name, design.originalName, ...design.tags]
            .join(' ')
            .toLocaleLowerCase()
            .includes(term),
        )
        .map((design) => ({
          id: `design-${design.id}`,
          view: 'Biblioteca de diseños' as View,
          title: design.name,
          meta: 'Diseño guardado',
        })),
    ].slice(0, 8);
  }, [data, globalQuery]);
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (response.status === 401) {
        window.location.assign('/ingresar?return_to=%2F');
        return;
      }
      const result = await readJsonResponse<AppData & { error?: string }>(
        response,
      );
      if (!response.ok)
        throw new Error(result.error || 'No pudimos cargar la información');
      setData(result);
      setError('');
    } catch (cause) {
  ...
  useEffect(() => {
    // oxlint-disable-next-line react-compiler/react-compiler -- Initial asynchronous data synchronization.
    void refresh();
  }, [refresh]);
        ...
        <header className="sticky top-0 z-30 flex h-[72px] items-center gap-3 border-b border-border bg-background/90 px-4 backdrop-blur-xl sm:px-7">
          ...
          <label className="relative hidden max-w-[420px] flex-1 sm:block">
            ...
            {globalQuery.trim().length >= 2 && (
              <div className="absolute left-0 right-0 top-12 z-50 overflow-hidden rounded-xl border bg-popover p-1 shadow-xl">
        ...
        <section className="mx-auto max-w-[1500px] p-4 sm:p-7 xl:p-9">
          {loading ? (
            ...
            <>
              ...
              {active === 'Biblioteca de diseños' && (
                <DesignLibrary
                  folders={data.designFolders}
                  designs={data.designs}
                  customers={data.customers}
                  mutate={mutate}
                  onUseInBuilder={(design) => {
                    setBuilderSeed(design);
                    go('Armador DTF');
                  }}
                />
              )}
              {active === 'Armador DTF' && (
                <DtfBuilder
                  seed={builderSeed}
                  onSeedConsumed={() => setBuilderSeed(null)}
                  openLibrary={() => go('Biblioteca de diseños')}
                />
              )}
...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      >
        {nav.map((item) => (
          ...
          >
            ...
            {item.label === 'Biblioteca de diseños' && designsCount > 0 && (
              <span
                className={`rounded-full px-2 py-0.5 text-[11px] ${active === item.label ? 'bg-white/20' : 'bg-muted'}`}
              >
                {designsCount}
      ...
      <div className="m-3 rounded-2xl border border-[#bfece5] bg-[#e9fbf7] p-4">
        <div className="flex items-center gap-2 text-xs font-black text-[#116c61]">
          <Lock className="size-4" /> Información privada
        </div>
        <p className="mt-2 text-xs leading-5 text-[#47736e]">
          {ordersCount} pedidos · {designsCount} diseños guardados
        </p>
      </div>
    </aside>
  );
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          ...
          meta={`${openOrders.filter((order) => order.dueDate && order.dueDate <= nextWeekDate).length} para los próximos 7 días`}
          tint="cyan"
        />
        <Metric
        ...
        <Metric
          ...
          value={String(data.designs.length)}
          meta={`${data.designFolders.length} carpetas en la biblioteca`}
          tint="green"
        />
      </div>
      ...
      <section className="mt-5 rounded-2xl border bg-card p-5 shadow-soft">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <QuickAction
            ...
            meta={`${data.designs.length} diseños disponibles`}
            onClick={() => go('Biblioteca de diseños')}
          />
          <QuickAction
            icon={FileCheck2}
            title="Ver aprobaciones"
            meta="Enlaces para clientes"
            onClick={() => go('Aprobaciones')}
          />
        </div>
      </section>
    </>
            ...
            <tbody className="divide-y">
              {products.map((product) => (
                <tr key={product.id} className="hover:bg-muted/35">
                  ...
                  {/* oxlint-disable-next-line jsx-a11y/control-has-associated-label -- This is a non-interactive stock table cell. */}
                  <td className="px-5 py-4">
              ...
              {!products.length && (
                <tr>
                  <td colSpan={5}>
                    <EmptyLine text="Todavía no agregaste productos al inventario." />
                  </td>
                </tr>
              )}
            </tbody>
  ...
  const toolCards = [
    {
      ...
      text: `${orders.filter((o) => o.status !== 'Entregado').length} pedidos todavía no fueron entregados`,
      tone: 'cyan',
    },
    {
      icon: Clock3,
      title: 'Pendientes de diseño',
      text: `${orders.filter((o) => ['Consulta', 'Esperando seña', 'Diseño pendiente'].includes(o.status)).length} pedidos esperan definición o aprobación`,
      tone: 'violet',
    },
    {
      icon: AlertTriangle,
    ...
    },
    {
      ...
      text: `${orders.filter((order) => order.status === 'Listo para entregar').length} pedidos esperan retiro o envío`,
      tone: 'cyan',
    },
  ];
  return (
...
function DtfBuilder({
  ...
  useEffect(() => {
    if (!seed) return;
    const url = designFileUrl(seed.fileKey);
    const image = new window.Image();
    image.onload = () => {
      const naturalWidth = seed.widthPx || image.naturalWidth;
      const naturalHeight = seed.heightPx || image.naturalHeight;
      const aspectRatio = naturalWidth / naturalHeight || 1;
      setDesigns((current) => [
        ...current,
        {
          id: `library-${seed.id}-${crypto.randomUUID()}`,
          name: seed.name,
          url,
          width: 18,
          height: Math.max(0.1, Math.round((18 / aspectRatio) * 10) / 10),
          quantity: 1,
          naturalWidth,
          aspectRatio,
          lockAspect: true,
    ...
    image.onerror = () => {
      setBuilderError(
        'No se pudo abrir el diseño guardado. Probá descargarlo y volver a subirlo.',
      );
      onSeedConsumed();
    };
    image.src = url;
  ...
  const upload = (files: FileList | null) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      image.onerror = () => {
        ...
        setBuilderError(
          `No se pudo leer ${file.name}. Verificá que sea una imagen válida.`,
        );
      };
      image.src = url;
    });
      ...
      <div className="grid gap-5 xl:grid-cols-[330px_1fr_260px]">
        <section className="rounded-2xl border bg-card p-4 shadow-soft">
          ...
          <Button
            ...
            onClick={openLibrary}
          >
            <FolderOpen /> Elegir de la biblioteca
          </Button>
    ...
    <Dialog open={open} onOpenChange={(value) => !value && close()}>
      <DialogContent className="sm:max-w-[560px]">
        ...
        >
          ...
          {formError && (
            <p className="rounded-xl bg-red-50 p-3 text-xs text-red-800 sm:col-span-2">
              {formError}
            </p>
          )}
        </form>
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => !value && close()}>
      <DialogContent className="sm:max-w-[620px]">
        ...
        >
          <Field label="Plantilla rápida" wide>
            <NativeSelect
              className="w-full"
              defaultValue=""
              onChange={(event) => {
                ...
                (form.elements.namedItem('sizes') as HTMLInputElement).value =
                  template.sizes;
              }}
            >
...
function SearchBox({
  ...
  return (
    <div className="relative block max-w-[460px] flex-1">
      <Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
      <Input
